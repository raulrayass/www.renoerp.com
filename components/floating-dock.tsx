...
export function FloatingDock() {
  ...
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50 lg:hidden px-3">
      <div className="flex items-center gap-1 px-2 py-1.5 rounded-2xl bg-card border border-border shadow-lg backdrop-blur-sm">
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              href={href}
              className={cn(
                'flex items-center justify-center w-7 h-7 rounded-lg transition-all duration-200 p-0',
                active
